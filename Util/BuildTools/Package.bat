...
:: 如果游戏模式不为carla，则切换到对应模式
if not "%GAME_MODE%"=="car" (
    echo Switching to game mode: %GAME_MODE%
    where conda
    call conda env list
...
if %DO_COPY_FILES%==true (
    ...
    :: AirSim 联合仿真多载具配置文件
    if exist "!XCOPY_FROM!AirSimConfig" (
        echo d | xcopy /y /s "!XCOPY_FROM!AirSimConfig"                             "!XCOPY_TO!AirSimConfig"
    )
    :: AirSim 联合仿真示例与测试套件
    ...
    if exist "!XCOPY_FROM!CarlaAir.ps1" (
